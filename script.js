fetch('asserts/file-list.json')
  ...
  .then(data => {
    ...
    data.posts.forEach(post => {
      ...
      const fetch('asserts/file-list.json')
      .then(response => response.json())
      .then(data => {
        // 動態載入圖片
        const galleryContainer = document.getElementById('gallery-container');
        ...
        data.posts.forEach(post => {
          ...
          pathElement.textContent = `路徑: ${post.path} -> ${formattedPath}`;
    ...
    updatedTimeElement = document.createElement('span');
      updatedTimeElement.classList.add('updated-time');
      updatedTimeElement.textContent = `最後更新時間: ${updatedTime}`;
      
      // 把文章標題和修改時間加入到 list item
      li.appendChild(a);
      li.appendChild(b.textContent);
      li.appendChild(updatedTimeElement);
